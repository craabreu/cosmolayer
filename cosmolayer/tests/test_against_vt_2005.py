...
from cosmolayer.sac import GAS_CONSTANT, create_cosmo_sac_2002_matrix
...
def interaction_matrix() -> NDArray[np.float64]:
    # Get the dimensionless interaction matrix (ΔW/RT) at reference temperature
    U_RT = create_cosmo_sac_2002_matrix(_REF_TEMP)
    # Convert back to energy units (kcal/mol) for compatibility with tests
    delta_w: NDArray[np.float64] = U_RT * (GAS_CONSTANT * _REF_TEMP)
    return delta_w
