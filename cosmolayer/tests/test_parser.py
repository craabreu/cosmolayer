"""Unit tests for ``cosmolayer.parser.get_atom_mapped_smiles``."""
...
from cosmolayer.parser import get_atom_mapped_smiles, parse_cosmo_file


def _smiles_to_mol_keep_hs(smiles: str) -> Chem.Mol:
...
class TestGetAtomMappedSmiles:
    def test_returns_atom_mapped_smiles_for_water(
        ...
        smiles = get_atom_mapped_smiles(water_atoms)
        assert smiles == "[O:1]([H:2])[H:3]"

    def test_returns_atom_mapped_smiles_for_fluoromethane(
        self, fluoromethane_atoms: pd.DataFrame
    ) -> None:
        smiles = get_atom_mapped_smiles(fluoromethane_atoms)
        assert smiles == "[C:1]([F:2])([H:3])([H:4])[H:5]"

    def test_result_is_valid_smiles(self, fluoromethane_atoms: pd.DataFrame) -> None:
        smiles = get_atom_mapped_smiles(fluoromethane_atoms)
        assert smiles is not None
        assert _smiles_to_mol_keep_hs(smiles) is not None
    ...
    def test_atom_map_numbers_are_one_indexed_and_unique(
        ...
        smiles = get_atom_mapped_smiles(fluoromethane_atoms)
        assert smiles is not None
        mol = _smiles_to_mol_keep_hs(smiles)
